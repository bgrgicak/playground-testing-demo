...
describe('Workshop Tests', () => {
  ...
  beforeEach(async () => {
    cliServer = await runCLI({
      command: 'server',
      mount: [
        '.:/wordpress/wp-content/plugins/workshop-building-automated-tests-with-wordpress-playground',
      ...
      blueprint: {
        steps: [
          {
            step: 'activatePlugin',
            pluginPath: '/wordpress/wp-content/plugins/workshop-building-automated-tests-with-wordpress-playground',
          }
        ],
      },
    });
  ...
  test('Should activate plugin', async () => {
    ...
    expect(activePlugins.json).toContain(
      'workshop-building-automated-tests-with-wordpress-playground/workshop-building-automated-tests-with-wordpress-playground.php'
    );
  });
    ...
    const result = await php.run({
      code: `
        ...
        echo json_encode(array(
          'message' => wbatwp_hello_response_message('John Doe')
        ));
      `,
    });
  ...
  test('Should save message after API request', async () => {
    ...
    const result = await php.run({
      code: `
        ...
        require_once '/wordpress/wp-load.php';
        echo json_encode(array(
          'message' => get_option(WBATWP_OPTIONS_KEY)
        ));
      `,
    });
